...
export const GET: APIRoute = async ({ request }) => {
  ...
  return json({
    ...
    endpoints: {
      ...
      'GET /api/v1/crm/households':       'List CRM families ("households"). Optional ?q=, ?limit=, ?after=.',
      'POST /api/v1/crm/households':      'Create a family. Body: { name, street?, city?, postal_code?, notes?, owner_email? }. Returns { id }.',
      'GET /api/v1/crm/households/:id':   'Get a family, with its members.',
      'PATCH /api/v1/crm/households/:id': 'Update a family. Body: HouseholdInput with owner_email? instead of owner_employee_id.',
      'POST /api/v1/crm/households/:id/members':   'Add/move a contact into the family. Body: { contact_id, role }.',
      'DELETE /api/v1/crm/households/:id/members': 'Remove a contact from the family. Body: { contact_id }.',
      'GET /api/v1/crm/contacts/:id/links':    'List a contact\'s links.',
      'POST /api/v1/crm/contacts/:id/links':   'Link two contacts. Body: { to_contact_id, type, note? }. Returns { id, suggest_family }.',
      'DELETE /api/v1/crm/contacts/:id/links': 'Remove a link. Body: { id }.',
      'PUT /api/v1/crm/contacts/:id/tags':     'Replace a contact\'s tags. Body: { tags: string[] } (tag ids or names).',
      'POST /api/v1/crm/contacts/:id/tags':    'Add a tag to a contact. Body: { tag } (id or name).',
      'DELETE /api/v1/crm/contacts/:id/tags':  'Remove a tag from a contact. Body: { tag } (id or name).',
      'GET /api/v1/crm/tags':              'List CRM tags.',
      'POST /api/v1/crm/tags':             'Create a tag (admin only). Body: { name, color? }. Returns { id }.',
      'GET /api/v1/crm/contacts/:id/activities':    'List a contact\'s timeline. Optional ?limit=, ?before= (occurred_at cursor). Returns { activities, next }.',
      'POST /api/v1/crm/contacts/:id/activities':   'Log an activity. Body: { kind, body, occurred_at? }. Returns { id }.',
      'PATCH /api/v1/crm/activities/:id':  'Update an activity (author or admin; never kind system). Body: { body?, kind?, occurred_at? }.',
      'DELETE /api/v1/crm/activities/:id': 'Delete an activity (author or admin; never kind system).',
      'GET /api/v1/crm/contacts/:id/tasks':   'List a contact\'s tasks. Optional ?include_done=1. Returns { tasks }.',
      'POST /api/v1/crm/contacts/:id/tasks':  'Create a task on a contact. Body: { title, due_date?, assignee_email? }. Returns { id }.',
      'GET /api/v1/crm/tasks':             'List tasks across contacts. Optional ?assignee=<email>|all (default: the caller; the service account with no filter defaults to all), ?include_done=1, ?limit=.',
      'PATCH /api/v1/crm/tasks/:id':       'Update a task. Body: { title?, due_date?, assignee_email?, done? }.',
      'DELETE /api/v1/crm/tasks/:id':      'Delete a task.',
      'GET /api/v1/crm/contacts/:id/dates':   'List a contact\'s dates, each with Hebrew and next-occurrence info. Returns { dates }.',
      'POST /api/v1/crm/contacts/:id/dates':  'Add a date. Body: { kind, date? (ISO) | hebrew?: { day, month, year }, label? }. Returns { id, date }.',
      'PATCH /api/v1/crm/dates/:id':       'Update a date. Body: { kind?, date?, label? }.',
      'DELETE /api/v1/crm/dates/:id':      'Delete a date.',
      'GET /api/v1/crm/upcoming':          'Upcoming dates across visible contacts. Optional ?days= (default 30), ?limit=. Returns { dates }.',
      'GET /api/v1/crm/hebrew-date':       'Convert between Gregorian and Hebrew dates. ?date=YYYY-MM-DD[&after_sunset=1], or ?day=&month=&year=.',
